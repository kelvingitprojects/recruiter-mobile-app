...
export default function OnboardingCandidate({ navigation }) {
  ...
  const submit = () => {
    dispatch(completeOnboarding({ title, skills: skills.split(',').map(s => s.trim()) }));
  };
  return (
    <View style={styles.container}>
      <View style={styles.roleRow}>
        <Pressable style={[styles.roleCard, styles.roleActive]}>
          ...
          <Text style={[styles.roleText, styles.roleTextActive]}>Candidate</Text>
        </Pressable>
        <Pressable onPress={() => { dispatch(setRole('recruiter')); navigation.replace('OnboardingRecruiter'); }} style={styles.roleCard}>
          <Ionicons name="briefcase" size={20} color={colors.muted} />
